class Evals
  ...
  def self.explode(radius, damage); "aoe(#{radius}, 'Evals.damage_to_coord(coord, #{damage})'); destroy"; end
  def self.damage_to_coord(coord, damage)
    unless Dungeon.at(coord).is_solid?
      VisualEffect.new("Fire Blast Effect", "* ".color(:light_red), coord)
  ...
  def self.poison_tick(damage); "dot.effector.hurt(#{damage}) if dot.effector.is_a?(Creature)"; end
end
